...
import { type FC, useCallback, useEffect, useMemo, useReducer, useRef } from 'react'
...
const TextureShaderMaterial = extend(TextureShader)
...
const TextureScene: FC = () => {
  ...
  const shaderRef = useRef<
    (BackgroundShaderUniforms & InstanceType<typeof TextureShader>) | null
  >(null)
  ...
  const exportSettingsRef = useRef<{ seed: number; resolution: number }>({
    seed: 0,
    resolution: 2048,
  })
  const [captureTick, requestCapture] = useReducer((value: number) => value + 1, 0)
  ...
  const captureHighResTexture = useCallback(() => {
    const targetCamera = cameraRef.current

    if (!gl || !shaderRef.current || !targetCamera) {
      console.error('Canvas not ready for capture')
      return
    }

    try {
      // Use latest export settings snapshot
      const { resolution: targetResolution, seed } = exportSettingsRef.current
      ...
      targetCamera.left = -1
      targetCamera.right = 1
      targetCamera.top = 1
      targetCamera.bottom = -1
      targetCamera.updateProjectionMatrix()

      // Render single frame at high resolution
      gl.render(scene, targetCamera)
  ...
  useEffect(() => {
    exportSettingsRef.current.seed = exportSettings.seed
    exportSettingsRef.current.resolution = exportSettings.resolution
  }, [exportSettings.resolution, exportSettings.seed])

  useEffect(() => {
    if (captureTick === 0) return
    captureHighResTexture()
  }, [captureHighResTexture, captureTick])
  ...
  useFrame((_, delta) => {
    ...
    shaderRef.current.uPaletteIndex = 0
    shaderRef.current.uSampleWeight = 1

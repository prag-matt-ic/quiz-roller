...
const Badge: FC<BadgeProps> = ({
  ...
  const ringClass = withRing ? 'ring ring-teal-accent/25' : ''
  ...
  return (
    <div
      className={twMerge(
        `badge inline-flex w-fit items-center rounded-full bg-black/80 font-medium select-none`,
        containerSize,
        withRing && ringClass,
        className,
      )}>
      <Icon className={twJoin('text-teal-accent')} size={iconSize} strokeWidth={1.75} />
      <span
        className={twJoin(
